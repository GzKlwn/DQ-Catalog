...
def fix_description_format(rule_path, folder_name):
    """Fix Description section to use bold text format instead of headings"""
    ...
    # NEW FORMAT - using **bold** instead of ### headings
...
"""
    ...
    # Replace old description section (heading format) with new one (bold format)
    # Match from ## Description to ## Output Fields or ## Logic
    ...
    if re.search(old_desc_pattern, content, re.DOTALL):
        ...
        # If pattern doesn't match, just replace the whole section
        content = re.sub(
            r'## Description.*?(?=\n## Output Fields|\n## Logic)',
...
print("Converting Description section format for webapp parser...")
print("(### headings -> **bold** text)\n")
...
for folder in sorted(os.listdir(rules_dir)):
    if folder.startswith('ACE_DQ_'):
        ...
        if os.path.isfile(rule_path):
            if fix_description_format(rule_path, folder):
                fixed += 1

print(f"Fixed Description format for {fixed} rules")
print("\nFormat change:")
print("  FROM: ### 1. Functional/Business Description")
print("  TO:   **1. Functional/Business Description**")
print("\nWebapp parser will now correctly extract:")
print("  - Description")
print("  - Fetch")
print("  - Check")
print("  - Return")
