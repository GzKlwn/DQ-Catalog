# DQ Rule: ACE_00060 - Sales orders MUST NOT be marked as delivery-complete when any order line still requires delivery.
...
|-------|-------|
| Rule ID | 00060 |
| Rule Name | Sales orders MUST NOT be marked as delivery-complete when any order line still requires delivery. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0068_System_001_KEKO_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: sales orders must not be marked as delivery-complete when any order line still requires delivery.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in VBAK, evaluates the defined error conditions: sales orders must not be marked as delivery-complete when any order line still requires delivery.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | KEKO | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | KEKO | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | KEKO | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | KEKO | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Sales orders MUST NOT be marked as delivery-complete when any order line still requires delivery.
-- Rule ID: ACE_00060
...
CREATE VIEW [dbo].[DQ_ACE00060_System_001_KEKO_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00060_System_001_KEKO_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00060_System_001_KEKO_OptSel]
WHERE
