# DQ Rule: ACE_00044 - Open payable documents MUST NOT have a due date earlier than the posting date.
...
|-------|-------|
| Rule ID | 00044 |
| Rule Name | Open payable documents MUST NOT have a due date earlier than the posting date. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0052_System_001_EKBE_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: open payable documents must not have a due date earlier than the posting date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in UNKNOWN, evaluates the defined error conditions: open payable documents must not have a due date earlier than the posting date.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Open payable documents MUST NOT have a due date earlier than the posting date.
-- Rule ID: ACE_00044
...
CREATE VIEW [dbo].[DQ_ACE00044_System_001_EKBE_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00044_System_001_EKBE_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00044_System_001_EKBE_OptSel]
WHERE
