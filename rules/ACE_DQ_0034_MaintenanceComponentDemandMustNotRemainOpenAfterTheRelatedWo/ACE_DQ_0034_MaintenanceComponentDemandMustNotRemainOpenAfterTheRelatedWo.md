# DQ Rule: ACE_00034 - Maintenance component demand MUST NOT remain open after the related work has reached technical completion.
...
|-------|-------|
| Rule ID | 00034 |
| Rule Name | Maintenance component demand MUST NOT remain open after the related work has reached technical completion. |
| Rule Name Score | 100% |
| Data Domain | Internal Orders |
| Object Type | Master |
...
| Generated View | DQ_0042_System_001_IMPTT_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: maintenance component demand must not remain open after the related work has reached technical completion.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.

**2. Specific Relevancy Criteria/Scope**

- **Primary Table:** AUFM
- **Related Tables:** AUFK
- **Domain:** Internal Orders
- **Scope:** All active records in AUFM
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from AUFM that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in AUFM, evaluates the defined error conditions: maintenance component demand must not remain open after the related work has reached technical completion.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | IMPTT | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | IMPTT | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | IMPTT | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | IMPTT | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Maintenance component demand MUST NOT remain open after the related work has reached technical completion.
-- Rule ID: ACE_00034
...
CREATE VIEW [dbo].[DQ_ACE00034_System_001_IMPTT_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00034_System_001_IMPTT_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00034_System_001_IMPTT_OptSel]
WHERE
