# DQ Rule: ACE_00033 - Physical inventory documents MUST NOT be posted before the recorded count date.
...
|-------|-------|
| Rule ID | 00033 |
| Rule Name | Physical inventory documents MUST NOT be posted before the recorded count date. |
| Rule Name Score | 100% |
| Data Domain | Physical Inventory |
| Object Type | Master |
...
| Generated View | DQ_0041_System_001_IMPTT_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: physical inventory documents must not be posted before the recorded count date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Scope:** All active records in MKPF
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from MKPF that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in MKPF, evaluates the defined error conditions: physical inventory documents must not be posted before the recorded count date.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Physical inventory documents MUST NOT be posted before the recorded count date.
-- Rule ID: ACE_00033
...
CREATE VIEW [dbo].[DQ_ACE00033_System_001_IMPTT_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00033_System_001_IMPTT_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00033_System_001_IMPTT_OptSel]
WHERE
