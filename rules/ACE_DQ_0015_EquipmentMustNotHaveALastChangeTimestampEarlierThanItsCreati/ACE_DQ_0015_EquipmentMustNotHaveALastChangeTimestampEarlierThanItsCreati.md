# DQ Rule: ACE_00015 - Equipment MUST NOT have a last change timestamp earlier than its creation date.
...
|-------|-------|
| Rule ID | 00015 |
| Rule Name | Equipment MUST NOT have a last change timestamp earlier than its creation date. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0023_System_001_IFLOT_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: equipment must not have a last change timestamp earlier than its creation date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in EQUI, evaluates the defined error conditions: equipment must not have a last change timestamp earlier than its creation date.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | IFLOT | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | IFLOT | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | IFLOT | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | IFLOT | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Equipment MUST NOT have a last change timestamp earlier than its creation date.
-- Rule ID: ACE_00015
...
CREATE VIEW [dbo].[DQ_ACE00015_System_001_IFLOT_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00015_System_001_IFLOT_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00015_System_001_IFLOT_OptSel]
WHERE
