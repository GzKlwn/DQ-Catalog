# DQ Rule: ACE_00040 - Counter overflow reading MUST exceed the maximum total counter reading.
...
|-------|-------|
| Rule ID | 00040 |
| Rule Name | Counter overflow reading MUST exceed the maximum total counter reading. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0048_System_001_QALS_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: counter overflow reading must exceed the maximum total counter reading.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in UNKNOWN, evaluates the defined error conditions: counter overflow reading must exceed the maximum total counter reading.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Counter overflow reading MUST exceed the maximum total counter reading.
-- Rule ID: ACE_00040
...
CREATE VIEW [dbo].[DQ_ACE00040_System_001_QALS_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00040_System_001_QALS_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00040_System_001_QALS_OptSel]
WHERE
