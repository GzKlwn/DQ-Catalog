# DQ Rule: ACE_00065 - Production BOM items MUST have a positive required quantity when a component is specified.
...
|-------|-------|
| Rule ID | 00065 |
| Rule Name | Production BOM items MUST have a positive required quantity when a component is specified. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0073_System_001_BPGE_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: production bom items must have a positive required quantity when a component is specified.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in MAST, evaluates the defined error conditions: production bom items must have a positive required quantity when a component is specified.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | BPGE | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | BPGE | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | BPGE | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | BPGE | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Production BOM items MUST have a positive required quantity when a component is specified.
-- Rule ID: ACE_00065
...
CREATE VIEW [dbo].[DQ_ACE00065_System_001_BPGE_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00065_System_001_BPGE_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00065_System_001_BPGE_OptSel]
WHERE
