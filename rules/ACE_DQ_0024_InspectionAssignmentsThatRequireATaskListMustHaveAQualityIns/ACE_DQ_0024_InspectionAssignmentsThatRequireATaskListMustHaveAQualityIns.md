# DQ Rule: ACE_00024 - Inspection assignments that require a task list MUST have a quality inspection plan assigned for the same material and plant.
...
|-------|-------|
| Rule ID | 00024 |
| Rule Name | Inspection assignments that require a task list MUST have a quality inspection plan assigned for the same material and plant. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0032_System_001_AUFK_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: inspection assignments that require a task list must have a quality inspection plan assigned for the same material and plant.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in MARA, evaluates the defined error conditions: inspection assignments that require a task list must have a quality inspection plan assigned for the same material and plant.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | AUFK | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | AUFK | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | AUFK | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | AUFK | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Inspection assignments that require a task list MUST have a quality inspection plan assigned for the same material and plant.
-- Rule ID: ACE_00024
...
CREATE VIEW [dbo].[DQ_ACE00024_System_001_AUFK_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00024_System_001_AUFK_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00024_System_001_AUFK_OptSel]
WHERE
