# DQ Rule: ACE_00094 - Serialized delivery records MUST NOT carry a different transaction date than their linked serial assignments.
...
|-------|-------|
| Rule ID | 00094 |
| Rule Name | Serialized delivery records MUST NOT carry a different transaction date than their linked serial assignments. |
| Rule Name Score | 100% |
| Data Domain | Delivery Headers |
| Object Type | Master |
...
| Generated View | DQ_0102_System_001_MAPL_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: serialized delivery records must not carry a different transaction date than their linked serial assignments.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.

**2. Specific Relevancy Criteria/Scope**

- **Primary Table:** LIKP
- **Related Tables:** LIPS
- **Domain:** Delivery Headers
- **Scope:** All active records in LIKP
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from LIKP that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in LIKP, evaluates the defined error conditions: serialized delivery records must not carry a different transaction date than their linked serial assignments.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | MAPL | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | MAPL | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | MAPL | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | MAPL | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Serialized delivery records MUST NOT carry a different transaction date than their linked serial assignments.
-- Rule ID: ACE_00094
...
CREATE VIEW [dbo].[DQ_ACE00094_System_001_MAPL_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00094_System_001_MAPL_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00094_System_001_MAPL_OptSel]
WHERE
