# DQ Rule: ACE_00092 - Pricing records MUST have a positive pricing unit when a rate or amount is maintained.
...
|-------|-------|
| Rule ID | 00092 |
| Rule Name | Pricing records MUST have a positive pricing unit when a rate or amount is maintained. |
| Rule Name Score | 100% |
| Data Domain | Pricing Records |
| Object Type | Master |
...
| Generated View | DQ_0100_System_001_EKPO_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: pricing records must have a positive pricing unit when a rate or amount is maintained.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.

**2. Specific Relevancy Criteria/Scope**

- **Primary Table:** KONV
- **Related Tables:** N/A
- **Domain:** Pricing Records
- **Scope:** All active records in KONV
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from KONV that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in KONV, evaluates the defined error conditions: pricing records must have a positive pricing unit when a rate or amount is maintained.. Flags records that violate the rule requirements.
...
| 2 | zConcatenatedKey | | Calculated | | CONCAT: base.[zSourceSystemID] + '_EBELN' + ... | Yes |
...
|---|---------|-------------|-------------|-------------|
| 1 | EBELN | base.[EBELN] | | Primary key component |
| 2 | EBELP | base.[EBELP] | | Key component |
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | EKPO | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | EKPO | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | EKPO | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | EKPO | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Pricing records MUST have a positive pricing unit when a rate or amount is maintained.
-- Rule ID: ACE_00092
...
CREATE VIEW [dbo].[DQ_ACE00092_System_001_EKPO_OptSel] AS
...
SELECT
    ...
    /* Basic Fields */
        evaluated_items.[EBELN],
    evaluated_items.[EBELP],
...
CREATE VIEW [dbo].[DQ_ACE00092_System_001_EKPO_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00092_System_001_EKPO_OptSel]
WHERE
