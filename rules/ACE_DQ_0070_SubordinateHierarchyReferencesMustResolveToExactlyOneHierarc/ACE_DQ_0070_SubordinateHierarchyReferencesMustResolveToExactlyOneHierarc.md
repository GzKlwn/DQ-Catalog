# DQ Rule: ACE_00070 - Subordinate hierarchy references MUST resolve to exactly one hierarchy definition.
...
|-------|-------|
| Rule ID | 00070 |
| Rule Name | Subordinate hierarchy references MUST resolve to exactly one hierarchy definition. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0078_System_001_KONH_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: subordinate hierarchy references must resolve to exactly one hierarchy definition.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in UNKNOWN, evaluates the defined error conditions: subordinate hierarchy references must resolve to exactly one hierarchy definition.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Subordinate hierarchy references MUST resolve to exactly one hierarchy definition.
-- Rule ID: ACE_00070
...
CREATE VIEW [dbo].[DQ_ACE00070_System_001_KONH_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00070_System_001_KONH_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00070_System_001_KONH_OptSel]
WHERE
