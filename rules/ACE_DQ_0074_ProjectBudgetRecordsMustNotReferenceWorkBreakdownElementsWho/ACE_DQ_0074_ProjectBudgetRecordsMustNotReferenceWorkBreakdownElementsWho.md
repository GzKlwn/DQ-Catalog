# DQ Rule: ACE_00074 - Project budget records MUST NOT reference work breakdown elements whose last recorded change predates their creation.
...
|-------|-------|
| Rule ID | 00074 |
| Rule Name | Project budget records MUST NOT reference work breakdown elements whose last recorded change predates their creation. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0082_System_001_EKAB_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: project budget records must not reference work breakdown elements whose last recorded change predates their creation.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in PROJ, evaluates the defined error conditions: project budget records must not reference work breakdown elements whose last recorded change predates their creation.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | EKAB | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | EKAB | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | EKAB | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | EKAB | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Project budget records MUST NOT reference work breakdown elements whose last recorded change predates their creation.
-- Rule ID: ACE_00074
...
CREATE VIEW [dbo].[DQ_ACE00074_System_001_EKAB_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00074_System_001_EKAB_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00074_System_001_EKAB_OptSel]
WHERE
