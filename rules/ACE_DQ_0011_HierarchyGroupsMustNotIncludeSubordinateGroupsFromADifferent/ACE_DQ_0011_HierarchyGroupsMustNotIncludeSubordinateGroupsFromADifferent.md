# DQ Rule: ACE_00011 - Hierarchy groups MUST NOT include subordinate groups from a different controlling area.
...
|-------|-------|
| Rule ID | 00011 |
| Rule Name | Hierarchy groups MUST NOT include subordinate groups from a different controlling area. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0019_System_001_LAGP_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: hierarchy groups must not include subordinate groups from a different controlling area.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in UNKNOWN, evaluates the defined error conditions: hierarchy groups must not include subordinate groups from a different controlling area.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | LAGP | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | LAGP | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | LAGP | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | LAGP | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Hierarchy groups MUST NOT include subordinate groups from a different controlling area.
-- Rule ID: ACE_00011
...
CREATE VIEW [dbo].[DQ_ACE00011_System_001_LAGP_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00011_System_001_LAGP_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00011_System_001_LAGP_OptSel]
WHERE
