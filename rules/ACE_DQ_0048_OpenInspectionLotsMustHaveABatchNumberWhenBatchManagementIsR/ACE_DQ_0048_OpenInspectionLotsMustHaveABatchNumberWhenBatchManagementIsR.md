# DQ Rule: ACE_00048 - Open inspection lots MUST have a batch number when batch management is required.
...
|-------|-------|
| Rule ID | 00048 |
| Rule Name | Open inspection lots MUST have a batch number when batch management is required. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0056_System_001_VBAK_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: open inspection lots must have a batch number when batch management is required.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in QAMR, evaluates the defined error conditions: open inspection lots must have a batch number when batch management is required.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Open inspection lots MUST have a batch number when batch management is required.
-- Rule ID: ACE_00048
...
CREATE VIEW [dbo].[DQ_ACE00048_System_001_VBAK_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00048_System_001_VBAK_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00048_System_001_VBAK_OptSel]
WHERE
