# DQ Rule: ACE_00016 - Equipment MUST NOT have an end-of-use date earlier than its start-up date.
...
|-------|-------|
| Rule ID | 00016 |
| Rule Name | Equipment MUST NOT have an end-of-use date earlier than its start-up date. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0024_System_001_QMAT_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: equipment must not have an end-of-use date earlier than its start-up date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in EQUI, evaluates the defined error conditions: equipment must not have an end-of-use date earlier than its start-up date.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | QMAT | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | QMAT | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | QMAT | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | QMAT | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Equipment MUST NOT have an end-of-use date earlier than its start-up date.
-- Rule ID: ACE_00016
...
CREATE VIEW [dbo].[DQ_ACE00016_System_001_QMAT_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00016_System_001_QMAT_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00016_System_001_QMAT_OptSel]
WHERE
