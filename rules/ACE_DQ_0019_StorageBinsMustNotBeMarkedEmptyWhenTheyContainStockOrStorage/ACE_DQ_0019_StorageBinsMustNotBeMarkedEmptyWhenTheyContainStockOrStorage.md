# DQ Rule: ACE_00019 - Storage bins MUST NOT be marked empty when they contain stock or storage units.
...
|-------|-------|
| Rule ID | 00019 |
| Rule Name | Storage bins MUST NOT be marked empty when they contain stock or storage units. |
| Rule Name Score | 100% |
| Data Domain | Storage Bins |
| Object Type | Master |
...
| Generated View | DQ_0027_System_001_AUFK_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: storage bins must not be marked empty when they contain stock or storage units.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Related Tables:** N/A
- **Domain:** Storage Bins
- **Scope:** All active records in LAGP
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from LAGP that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in LAGP, evaluates the defined error conditions: storage bins must not be marked empty when they contain stock or storage units.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Storage bins MUST NOT be marked empty when they contain stock or storage units.
-- Rule ID: ACE_00019
...
CREATE VIEW [dbo].[DQ_ACE00019_System_001_AUFK_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00019_System_001_AUFK_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00019_System_001_AUFK_OptSel]
WHERE
