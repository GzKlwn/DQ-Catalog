# DQ Rule: ACE_00080 - Vendor pricing records MUST specify a currency or percentage unit when a non-zero condition amount is maintained.
...
|-------|-------|
| Rule ID | 00080 |
| Rule Name | Vendor pricing records MUST specify a currency or percentage unit when a non-zero condition amount is maintained. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0088_System_001_MAPL_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: vendor pricing records must specify a currency or percentage unit when a non-zero condition amount is maintained.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Scope:** All active records in LFA1
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from LFA1 that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in LFA1, evaluates the defined error conditions: vendor pricing records must specify a currency or percentage unit when a non-zero condition amount is maintained.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Vendor pricing records MUST specify a currency or percentage unit when a non-zero condition amount is maintained.
-- Rule ID: ACE_00080
...
CREATE VIEW [dbo].[DQ_ACE00080_System_001_MAPL_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00080_System_001_MAPL_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00080_System_001_MAPL_OptSel]
WHERE
