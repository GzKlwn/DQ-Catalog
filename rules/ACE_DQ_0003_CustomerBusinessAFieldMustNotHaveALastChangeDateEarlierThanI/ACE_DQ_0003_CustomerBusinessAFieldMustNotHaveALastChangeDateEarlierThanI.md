# DQ Rule: ACE_00003 - Customer business a field MUST NOT have a last change date earlier than its creation date.
...
|-------|-------|
| Rule ID | 00003 |
| Rule Name | Customer business a field MUST NOT have a last change date earlier than its creation date. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0011_System_001_ADRC_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: customer business a field must not have a last change date earlier than its creation date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Scope:** All active records in KNA1
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from KNA1 that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in KNA1, evaluates the defined error conditions: customer business a field must not have a last change date earlier than its creation date.. Flags records that violate the rule requirements.
...
| 2 | zConcatenatedKey | | Calculated | | CONCAT: base.[zSourceSystemID] + '_ADDRNUMBER' + ... | Yes |
...
|---|---------|-------------|-------------|-------------|
| 1 | ADDRNUMBER | base.[ADDRNUMBER] | | Primary key component |
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | ADRC | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | ADRC | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | ADRC | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Customer business a field MUST NOT have a last change date earlier than its creation date.
-- Rule ID: ACE_00003
...
CREATE VIEW [dbo].[DQ_ACE00003_System_001_ADRC_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00003_System_001_ADRC_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00003_System_001_ADRC_OptSel]
WHERE
