# DQ Rule: ACE_00036 - Maintenance plans MUST have a maintenance strategy assigned before scheduling activity occurs.
...
|-------|-------|
| Rule ID | 00036 |
| Rule Name | Maintenance plans MUST have a maintenance strategy assigned before scheduling activity occurs. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0044_System_001_BKPF_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: maintenance plans must have a maintenance strategy assigned before scheduling activity occurs.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in AUFM, evaluates the defined error conditions: maintenance plans must have a maintenance strategy assigned before scheduling activity occurs.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | BKPF | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | BKPF | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | BKPF | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | BKPF | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Maintenance plans MUST have a maintenance strategy assigned before scheduling activity occurs.
-- Rule ID: ACE_00036
...
CREATE VIEW [dbo].[DQ_ACE00036_System_001_BKPF_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00036_System_001_BKPF_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00036_System_001_BKPF_OptSel]
WHERE
