# DQ Rule: ACE_00095 - Serialized delivery records MUST NOT reference different products across related serial and equipment assignments.
...
|-------|-------|
| Rule ID | 00095 |
| Rule Name | Serialized delivery records MUST NOT reference different products across related serial and equipment assignments. |
| Rule Name Score | 100% |
| Data Domain | Equipment Master |
| Object Type | Master |
...
| Generated View | DQ_0103_System_001_CRCA_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: serialized delivery records must not reference different products across related serial and equipment assignments.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.

**2. Specific Relevancy Criteria/Scope**

- **Primary Table:** EQUI
- **Related Tables:** EQKT
- **Domain:** Equipment Master
- **Scope:** All active records in EQUI
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from EQUI that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in EQUI, evaluates the defined error conditions: serialized delivery records must not reference different products across related serial and equipment assignments.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | CRCA | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | CRCA | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | CRCA | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | CRCA | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Serialized delivery records MUST NOT reference different products across related serial and equipment assignments.
-- Rule ID: ACE_00095
...
CREATE VIEW [dbo].[DQ_ACE00095_System_001_CRCA_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00095_System_001_CRCA_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00095_System_001_CRCA_OptSel]
WHERE
