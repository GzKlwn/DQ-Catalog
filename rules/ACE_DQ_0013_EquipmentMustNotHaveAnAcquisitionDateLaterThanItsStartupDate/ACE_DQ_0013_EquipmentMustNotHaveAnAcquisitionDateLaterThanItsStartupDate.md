# DQ Rule: ACE_00013 - Equipment MUST NOT have an acquisition date later than its start-up date.
...
|-------|-------|
| Rule ID | 00013 |
| Rule Name | Equipment MUST NOT have an acquisition date later than its start-up date. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0021_System_001_IFLOT_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: equipment must not have an acquisition date later than its start-up date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in EQUI, evaluates the defined error conditions: equipment must not have an acquisition date later than its start-up date.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Equipment MUST NOT have an acquisition date later than its start-up date.
-- Rule ID: ACE_00013
...
CREATE VIEW [dbo].[DQ_ACE00013_System_001_IFLOT_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00013_System_001_IFLOT_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00013_System_001_IFLOT_OptSel]
WHERE
