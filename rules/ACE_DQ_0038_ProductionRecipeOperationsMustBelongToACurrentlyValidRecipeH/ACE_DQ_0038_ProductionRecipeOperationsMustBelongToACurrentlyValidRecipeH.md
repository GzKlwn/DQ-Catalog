# DQ Rule: ACE_00038 - Production recipe operations MUST belong to a currently valid recipe header.
...
|-------|-------|
| Rule ID | 00038 |
| Rule Name | Production recipe operations MUST belong to a currently valid recipe header. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0046_System_001_QALS_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: production recipe operations must belong to a currently valid recipe header.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in AFKO, evaluates the defined error conditions: production recipe operations must belong to a currently valid recipe header.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Production recipe operations MUST belong to a currently valid recipe header.
-- Rule ID: ACE_00038
...
CREATE VIEW [dbo].[DQ_ACE00038_System_001_QALS_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00038_System_001_QALS_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00038_System_001_QALS_OptSel]
WHERE
