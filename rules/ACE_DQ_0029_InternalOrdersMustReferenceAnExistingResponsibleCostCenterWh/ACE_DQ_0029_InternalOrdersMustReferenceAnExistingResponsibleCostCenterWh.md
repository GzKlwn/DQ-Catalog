# DQ Rule: ACE_00029 - Internal orders MUST reference an existing responsible cost center when one is assigned.
...
|-------|-------|
| Rule ID | 00029 |
| Rule Name | Internal orders MUST reference an existing responsible cost center when one is assigned. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0037_System_001_MHIS_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: internal orders must reference an existing responsible cost center when one is assigned.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in AUFM, evaluates the defined error conditions: internal orders must reference an existing responsible cost center when one is assigned.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Internal orders MUST reference an existing responsible cost center when one is assigned.
-- Rule ID: ACE_00029
...
CREATE VIEW [dbo].[DQ_ACE00029_System_001_MHIS_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00029_System_001_MHIS_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00029_System_001_MHIS_OptSel]
WHERE
