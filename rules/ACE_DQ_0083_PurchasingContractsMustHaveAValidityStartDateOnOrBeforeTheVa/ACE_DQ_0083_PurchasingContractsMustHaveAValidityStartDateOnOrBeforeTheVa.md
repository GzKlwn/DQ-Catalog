# DQ Rule: ACE_00083 - Purchasing contracts MUST have a validity start date on or before the validity end date.
...
|-------|-------|
| Rule ID | 00083 |
| Rule Name | Purchasing contracts MUST have a validity start date on or before the validity end date. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0091_System_001_VBAK_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: purchasing contracts must have a validity start date on or before the validity end date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in UNKNOWN, evaluates the defined error conditions: purchasing contracts must have a validity start date on or before the validity end date.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Purchasing contracts MUST have a validity start date on or before the validity end date.
-- Rule ID: ACE_00083
...
CREATE VIEW [dbo].[DQ_ACE00083_System_001_VBAK_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00083_System_001_VBAK_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00083_System_001_VBAK_OptSel]
WHERE
