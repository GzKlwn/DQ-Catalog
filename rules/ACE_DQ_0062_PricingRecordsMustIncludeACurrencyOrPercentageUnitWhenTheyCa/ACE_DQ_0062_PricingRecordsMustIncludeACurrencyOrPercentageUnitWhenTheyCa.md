# DQ Rule: ACE_00062 - Pricing records MUST include a currency or percentage unit when they carry a non-zero condition amount.
...
|-------|-------|
| Rule ID | 00062 |
| Rule Name | Pricing records MUST include a currency or percentage unit when they carry a non-zero condition amount. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0070_System_001_SETHEADER_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: pricing records must include a currency or percentage unit when they carry a non-zero condition amount.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in KONV, evaluates the defined error conditions: pricing records must include a currency or percentage unit when they carry a non-zero condition amount.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | SETHEADER | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | SETHEADER | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | SETHEADER | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | SETHEADER | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Pricing records MUST include a currency or percentage unit when they carry a non-zero condition amount.
-- Rule ID: ACE_00062
...
CREATE VIEW [dbo].[DQ_ACE00062_System_001_SETHEADER_OptSel] AS
...
WITH evaluated_items AS (
    ...
    FROM [WRKDQ].[dbo].[SETHEADER] AS base
    WHERE
...
CREATE VIEW [dbo].[DQ_ACE00062_System_001_SETHEADER_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00062_System_001_SETHEADER_OptSel]
WHERE
