# DQ Rule: ACE_00066 - Planned orders MUST NOT use a production version above its maintained maximum lot size.
...
|-------|-------|
| Rule ID | 00066 |
| Rule Name | Planned orders MUST NOT use a production version above its maintained maximum lot size. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0074_System_001_BPGE_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: planned orders must not use a production version above its maintained maximum lot size.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in AFKO, evaluates the defined error conditions: planned orders must not use a production version above its maintained maximum lot size.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Planned orders MUST NOT use a production version above its maintained maximum lot size.
-- Rule ID: ACE_00066
...
CREATE VIEW [dbo].[DQ_ACE00066_System_001_BPGE_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00066_System_001_BPGE_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00066_System_001_BPGE_OptSel]
WHERE
