# DQ Rule: ACE_00053 - Open purchase orders MUST NOT be marked as delivery complete when receipt quantity is still outstanding.
...
|-------|-------|
| Rule ID | 00053 |
| Rule Name | Open purchase orders MUST NOT be marked as delivery complete when receipt quantity is still outstanding. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0061_System_001_KONH_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: open purchase orders must not be marked as delivery complete when receipt quantity is still outstanding.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in LIKP, evaluates the defined error conditions: open purchase orders must not be marked as delivery complete when receipt quantity is still outstanding.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Open purchase orders MUST NOT be marked as delivery complete when receipt quantity is still outstanding.
-- Rule ID: ACE_00053
...
CREATE VIEW [dbo].[DQ_ACE00053_System_001_KONH_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00053_System_001_KONH_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00053_System_001_KONH_OptSel]
WHERE
