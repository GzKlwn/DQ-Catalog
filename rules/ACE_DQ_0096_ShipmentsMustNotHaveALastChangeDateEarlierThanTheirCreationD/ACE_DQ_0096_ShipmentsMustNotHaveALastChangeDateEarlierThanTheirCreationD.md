# DQ Rule: ACE_00096 - Shipments MUST NOT have a last change date earlier than their creation date.
...
|-------|-------|
| Rule ID | 00096 |
| Rule Name | Shipments MUST NOT have a last change date earlier than their creation date. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0104_System_001_CRCA_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: shipments must not have a last change date earlier than their creation date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in UNKNOWN, evaluates the defined error conditions: shipments must not have a last change date earlier than their creation date.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Shipments MUST NOT have a last change date earlier than their creation date.
-- Rule ID: ACE_00096
...
CREATE VIEW [dbo].[DQ_ACE00096_System_001_CRCA_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00096_System_001_CRCA_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00096_System_001_CRCA_OptSel]
WHERE
