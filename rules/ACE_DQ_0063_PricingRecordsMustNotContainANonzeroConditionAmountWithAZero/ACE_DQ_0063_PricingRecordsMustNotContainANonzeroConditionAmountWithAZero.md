# DQ Rule: ACE_00063 - Pricing records MUST NOT contain a non-zero condition amount with a zero pricing unit.
...
|-------|-------|
| Rule ID | 00063 |
| Rule Name | Pricing records MUST NOT contain a non-zero condition amount with a zero pricing unit. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0071_System_001_AFKO_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: pricing records must not contain a non-zero condition amount with a zero pricing unit.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in KONV, evaluates the defined error conditions: pricing records must not contain a non-zero condition amount with a zero pricing unit.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | AFKO | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | AFKO | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | AFKO | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | AFKO | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Pricing records MUST NOT contain a non-zero condition amount with a zero pricing unit.
-- Rule ID: ACE_00063
...
CREATE VIEW [dbo].[DQ_ACE00063_System_001_AFKO_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00063_System_001_AFKO_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00063_System_001_AFKO_OptSel]
WHERE
