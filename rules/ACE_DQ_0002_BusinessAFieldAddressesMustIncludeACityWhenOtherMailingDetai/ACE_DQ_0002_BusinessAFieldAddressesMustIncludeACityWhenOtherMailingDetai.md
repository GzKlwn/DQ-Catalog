# DQ Rule: ACE_00002 - Business a field addresses MUST include a city when other mailing details are present.
...
|-------|-------|
| Rule ID | 00002 |
| Rule Name | Business a field addresses MUST include a city when other mailing details are present. |
| Rule Name Score | 100% |
| Data Domain | Business Partner Master |
| Object Type | Master |
...
| Generated View | DQ_0010_System_001_MAST_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: business a field addresses must include a city when other mailing details are present.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.

**2. Specific Relevancy Criteria/Scope**

- **Primary Table:** BUT020
- **Related Tables:** ADRC
- **Domain:** Business Partner Master
- **Scope:** All active records in BUT020
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from BUT020 that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in BUT020, evaluates the defined error conditions: business a field addresses must include a city when other mailing details are present.. Flags records that violate the rule requirements.
...
| 2 | zConcatenatedKey | | Calculated | | CONCAT: base.[zSourceSystemID] + '_STLNR' + ... | Yes |
...
|---|---------|-------------|-------------|-------------|
| 1 | STLNR | base.[STLNR] | | Primary key component |
| 2 | STLTY | base.[STLTY] | | Key component |
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | MAST | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | MAST | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | MAST | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | MAST | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Business a field addresses MUST include a city when other mailing details are present.
-- Rule ID: ACE_00002
...
CREATE VIEW [dbo].[DQ_ACE00002_System_001_MAST_OptSel] AS
...
SELECT
    ...
    /* Basic Fields */
        evaluated_items.[STLNR],
    evaluated_items.[STLTY],
...
CREATE VIEW [dbo].[DQ_ACE00002_System_001_MAST_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00002_System_001_MAST_OptSel]
WHERE
