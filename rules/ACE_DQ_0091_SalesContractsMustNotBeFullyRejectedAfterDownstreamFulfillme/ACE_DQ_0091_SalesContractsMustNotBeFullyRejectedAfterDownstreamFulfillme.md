# DQ Rule: ACE_00091 - Sales contracts MUST NOT be fully rejected after downstream fulfillment or billing has started.
...
|-------|-------|
| Rule ID | 00091 |
| Rule Name | Sales contracts MUST NOT be fully rejected after downstream fulfillment or billing has started. |
| Rule Name Score | 100% |
| Data Domain | Billing Documents |
| Object Type | Master |
...
| Generated View | DQ_0099_System_001_VTTK_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: sales contracts must not be fully rejected after downstream fulfillment or billing has started.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Scope:** All active records in VBRK
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from VBRK that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in VBRK, evaluates the defined error conditions: sales contracts must not be fully rejected after downstream fulfillment or billing has started.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | VTTK | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | VTTK | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | VTTK | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | VTTK | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Sales contracts MUST NOT be fully rejected after downstream fulfillment or billing has started.
-- Rule ID: ACE_00091
...
CREATE VIEW [dbo].[DQ_ACE00091_System_001_VTTK_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00091_System_001_VTTK_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00091_System_001_VTTK_OptSel]
WHERE
