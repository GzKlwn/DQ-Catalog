# DQ Rule: ACE_00045 - Open receivable documents MUST NOT have a due date earlier than the original document date.
...
|-------|-------|
| Rule ID | 00045 |
| Rule Name | Open receivable documents MUST NOT have a due date earlier than the original document date. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0053_System_001_EKBE_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: open receivable documents must not have a due date earlier than the original document date.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in UNKNOWN, evaluates the defined error conditions: open receivable documents must not have a due date earlier than the original document date.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | EKBE | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | EKBE | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | EKBE | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | EKBE | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Open receivable documents MUST NOT have a due date earlier than the original document date.
-- Rule ID: ACE_00045
...
CREATE VIEW [dbo].[DQ_ACE00045_System_001_EKBE_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00045_System_001_EKBE_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00045_System_001_EKBE_OptSel]
WHERE
