# DQ Rule: ACE_00057 - Open sales orders awaiting billing MUST NOT contain billable items with invalid pricing.
...
|-------|-------|
| Rule ID | 00057 |
| Rule Name | Open sales orders awaiting billing MUST NOT contain billable items with invalid pricing. |
| Rule Name Score | 100% |
| Data Domain | Sales Orders |
| Object Type | Master |
...
| Generated View | DQ_0065_System_001_MAST_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: open sales orders awaiting billing must not contain billable items with invalid pricing.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Related Tables:** VBAP
- **Domain:** Sales Orders
- **Scope:** All active records in VBAK
- **System Filter:** System_001 (as configured in Studio)

**3. DQ Checks (Conditions)**

- **Fetch** <br /> Retrieves all candidate records from VBAK that meet the scope criteria and filters based on the business rule requirements.

- **Check** <br /> For each record in VBAK, evaluates the defined error conditions: open sales orders awaiting billing must not contain billable items with invalid pricing.. Flags records that violate the rule requirements.
...
```sql
-- ============================================================
-- DQ Rule: Open sales orders awaiting billing MUST NOT contain billable items with invalid pricing.
-- Rule ID: ACE_00057
...
CREATE VIEW [dbo].[DQ_ACE00057_System_001_MAST_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00057_System_001_MAST_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00057_System_001_MAST_OptSel]
WHERE
