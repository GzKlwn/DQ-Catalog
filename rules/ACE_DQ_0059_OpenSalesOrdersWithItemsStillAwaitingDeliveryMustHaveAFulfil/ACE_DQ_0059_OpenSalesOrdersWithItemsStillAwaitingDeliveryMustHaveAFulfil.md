# DQ Rule: ACE_00059 - Open sales orders with items still awaiting delivery MUST have a fulfillment plant assigned.
...
|-------|-------|
| Rule ID | 00059 |
| Rule Name | Open sales orders with items still awaiting delivery MUST have a fulfillment plant assigned. |
| Rule Name Score | 100% |
...
| Generated View | DQ_0067_System_001_MKAL_OptSel |

## Description

**1. Functional/Business Description**

This rule validates data integrity by ensuring: open sales orders with items still awaiting delivery must have a fulfillment plant assigned.. Violations indicate records requiring investigation and remediation to maintain system consistency and data quality standards.
...
- **Check** <br /> For each record in VBAK, evaluates the defined error conditions: open sales orders with items still awaiting delivery must have a fulfillment plant assigned.. Flags records that violate the rule requirements.
...
|---------|----------|-------------|-------|-------------|
| Primary Condition | = or <> | MKAL | Various | Data quality validation per rule |

## Joins

| Element | Source | Target | Type | Key | Cardinality |
|---------|--------|--------|------|-----|-------------|
| System Filter | MKAL | All | Alignment | zSourceSystemID = 'System_001' | N/A |
...
|-------------|----------|-------|-------|-----------|
| System Filter | = | MKAL | zSourceSystemID | System_001 |

### Exclusions
| Description | Operator | Table | Field | Condition |
|-------------|----------|-------|-------|-----------|
| Exclude Deleted | <> | MKAL | LVORM | X |

## SQL -- OptSel

```sql
-- ============================================================
-- DQ Rule: Open sales orders with items still awaiting delivery MUST have a fulfillment plant assigned.
-- Rule ID: ACE_00059
...
CREATE VIEW [dbo].[DQ_ACE00059_System_001_MKAL_OptSel] AS
...
CREATE VIEW [dbo].[DQ_ACE00059_System_001_MKAL_RptSel] AS

SELECT
    *

FROM [dbo].[DQ_ACE00059_System_001_MKAL_OptSel]
WHERE
